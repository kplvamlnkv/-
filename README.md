...
Чтобы понять, **на каких признаках основаны решения модели**, мы применили метод интерпретации SHAP. Для обученного CatBoost-классификатора вычислены значения SHAP Value для каждого признака как в глобальном масштабе, так и для отдельных предсказаний. Визуальный анализ summary-графиков SHAP позволил выявить ключевые факторы, влияющие на классификацию Recon-атак. Ниже перечислены **топ-5 наиболее значимых признаков** и их интерпретация:
